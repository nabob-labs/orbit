...
pub fn is_prod_email_required(data: &ProdIntent, user_email: String) -> bool {
    ...
    let business_website_check = not_contains_string(data.business_website.as_deref(), "bospay")
        && not_contains_string(data.business_website.as_deref(), "orbit");
    let user_email_check = not_contains_string(Some(&user_email), "bospay");
