...
impl ForeignTryFrom<api_enums::Connector> for common_enums::RoutableConnectors {
    ...
    fn foreign_try_from(from: api_enums::Connector) -> Result<Self, Self::Error> {
        Ok(match from {
            ...
            api_enums::Connector::Bospaythreedsserver => {
                Err(common_utils::errors::ValidationError::InvalidValue {
                    message: "bospaythreedsserver is not a routable connector".to_string(),
                })?
            }
...
impl ForeignFrom<payments::MandateData> for orbit_domain_models::mandates::MandateData {
    fn foreign_from(d: payments::MandateData) -> Self {
        Self {
            customer_acceptance: d.customer_acceptance.map(|d| {
                orbit_domain_models::mandates::CustomerAcceptance {
                    ...
                    online: d
                        .online
                        .map(|d| orbit_domain_models::mandates::OnlineMandate {
            ...
            mandate_type: d.mandate_type.map(|d| match d {
                payments::MandateType::MultiUse(Some(i)) => {
                    orbit_domain_models::mandates::MandateDataType::MultiUse(Some(
                        orbit_domain_models::mandates::MandateAmountData {
                            amount: i.amount,
                            currency: i.currency,
                ...
                payments::MandateType::SingleUse(i) => {
                    orbit_domain_models::mandates::MandateDataType::SingleUse(
                        orbit_domain_models::mandates::MandateAmountData {
                            amount: i.amount,
                            currency: i.currency,
                            start_date: i.start_date,
...
impl From<&domain::Address> for orbit_domain_models::address::Address {
    fn from(address: &domain::Address) -> Self {
            ...
            && address.last_name.is_none()
        {
            ...
            Some(orbit_domain_models::address::AddressDetails {
        ...
        let phone_details = if address.phone_number.is_none() && address.country_code.is_none() {
            ...
            Some(orbit_domain_models::address::PhoneDetails {
    ...
    for api_models::admin::MerchantConnectorResponse
{
    ...
    fn foreign_try_from(item: domain::MerchantConnectorAccount) -> Result<Self, Self::Error> {
        ...
        let connector_account_details: orbit_domain_models::router_data::ConnectorAuthType =
...
#[cfg(feature = "v1")]
impl ForeignTryFrom<&HeaderMap> for orbit_domain_models::payments::HeaderPayload {
    type Error = error_stack::Report<errors::ApiErrorResponse>;
    ...
    )> for payments::PaymentsRequest
{
    ...
    fn foreign_try_from(
        ...
        let mut billing_address = billing
            .map(orbit_domain_models::address::Address::from)
