...
use orbit_domain_models::mandates::AcceptanceType;
    ...
    fn try_from(
        ...
        Ok(Self {
            ...
            return_url: item
                ...
                .router_return_url
                .clone()
                .unwrap_or_else(|| "https://bospay.in/".to_string()),
    ...
    for types::RouterData<F, T, types::PaymentsResponseData>
{
    ...
    fn try_from(
        ...
        let connector_metadata = connector_source_response
            .encode_to_value()
            .change_context(errors::ConnectorError::ResponseHandlingFailed)?;
        // We get pending as the status from stripe, but orbit should give it as requires_customer_action as
