    ...
    for PaymentIntentCreate
{
    ...
    async fn get_trackers<'a>(
        ...
        let batch_encrypted_data = domain_types::crypto_operation(
            key_manager_state,
            common_utils::type_name!(orbit_domain_models::payments::PaymentIntent),
            domain_types::CryptoOperation::BatchEncrypt(
                orbit_domain_models::payments::FromRequestEncryptablePaymentIntent::to_encryptable(
                    orbit_domain_models::payments::FromRequestEncryptablePaymentIntent {
        ...
        let encrypted_data =
             orbit_domain_models::payments::FromRequestEncryptablePaymentIntent::from_encryptable(batch_encrypted_data)
                .change_context(errors::ApiErrorResponse::InternalServerError)
                .attach_printable("Failed while encrypting payment intent details")?;

        let payment_intent_domain =
            orbit_domain_models::payments::PaymentIntent::create_domain_model_from_request(
                payment_id,
                merchant_account,
                profile,
                request.clone(),
