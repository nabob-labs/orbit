...
use orbit_domain_models::payments::payment_intent::CustomerData;
...
use orbit_interfaces::integrity::{CheckIntegrity, FlowIntegrity, GetIntegrityObject};
use josekit::jwe;
...
pub fn validate_amount_to_capture_and_capture_method(
    ...
    let option_net_amount = orbit_domain_models::payments::payment_attempt::NetAmount::from_payments_request_and_payment_attempt(
        request,
        payment_attempt,
    );
            ...
            Some(match customer_data {
                ...
                None => {
                    let new_customer = domain::Customer {
                        ...
                        updated_by: None,
                        version: orbit_domain_models::consts::API_VERSION,
                    };
...
pub async fn retrieve_payment_token_data(
    ...
    let token_data_result = token_data_string
        ...
        .change_context(errors::ApiErrorResponse::InternalServerError)
        .attach_printable("failed to deserialize orbit token data");
...
pub async fn store_in_vault_and_generate_ppmt(
    ...
    let key_for_orbit_token = payment_attempt.get_payment_method().map(|payment_method| {
    ...
    if let Some(key_for_orbit_token) = key_for_orbit_token {
        key_for_orbit_token
            .insert(
        ...
        (Some(data), Some(cus_id)) => {
            ...
            Ok(Some(
                match data.mandate_type.get_required_value("mandate_type")? {
                    orbit_domain_models::mandates::MandateDataType::SingleUse(data) => {
                        new_mandate
                    ...
                    orbit_domain_models::mandates::MandateDataType::MultiUse(op_data) => {
                        match op_data {
...
mod tests {
    ...
    fn test_authenticate_client_secret_session_not_expired() {
        let payment_intent = PaymentIntent {
            ...
            active_attempt: orbit_domain_models::RemoteStorageObject::ForeignID(
                "nopes".to_string(),
            ),
...
impl GooglePayTokenDecryptor {
    ...
    pub fn decrypt_token(
        ...
        orbit_domain_models::router_data::GooglePayDecryptedData,
    ...
    > {
        ...
        let decrypted_data: orbit_domain_models::router_data::GooglePayDecryptedData =
            decrypted
...
pub async fn get_payment_method_details_from_payment_token(
    ...
    let orbit_token = if let Some(token) = payment_attempt.payment_token.clone() {
        let redis_conn = state
            .store
            .get_redis_conn()
            .change_context(errors::ApiErrorResponse::InternalServerError)
    ...
    let token = orbit_token
        .ok_or(errors::ApiErrorResponse::InternalServerError)
        .attach_printable("missing orbit_token")?;
    match token {
...
pub enum UnifiedAuthenticationServiceFlow {
    ...
    ExternalAuthenticationInitiate {
        ...
        card: Box<orbit_domain_models::payment_method_data::Card>,
        token: String,
    },
