...
use orbit_domain_models::address::Address;
use kgraph_utils::{
    mca as mca_graph,
            ...
            .and_then(|mandate_data| {
                mandate_data
                    ...
                    .map(|mandate_type| match mandate_type {
                        orbit_domain_models::mandates::MandateDataType::SingleUse(_) => {
                            euclid_enums::MandateType::SingleUse
                        }
                        orbit_domain_models::mandates::MandateDataType::MultiUse(_) => {
...
pub async fn get_merchant_cgraph(
    ...
    profile_id: &common_utils::id_type::ProfileId,
    transaction_type: &api_enums::TransactionType,
) -> RoutingResult<Arc<orbit_constraint_graph::ConstraintGraph<euclid_dir::DirValue>>> {
    ...
    let cached_cgraph = CGRAPH_CACHE
        .get_val::<Arc<orbit_constraint_graph::ConstraintGraph<euclid_dir::DirValue>>>(
            CacheKey {
                key: key.clone(),
                prefix: state.tenant.redis_key_prefix.clone(),
            },
    ...
    for choice in chosen {
        ...
        let cgraph_eligible = cached_cgraph
            .check_value_validity(
                dir_val,
                &context,
                &mut orbit_constraint_graph::Memoization::new(),
                &mut orbit_constraint_graph::CycleCheck::new(),
                None,
            )
            .change_context(errors::RoutingError::KgraphAnalysisError)?;
