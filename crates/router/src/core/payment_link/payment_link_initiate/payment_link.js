  ...
  if (paymentDetails.display_sdk_only) {
    ...
    if (orderDetails !== null) {
      ...
      orderDetails.push({
        ...
        "product_img_link": "https://live.orbit.io/payment-link-assets/cart_placeholder.png",
...
/**
 * Uses
 *    - Creates and returns a div element with the Orbit branding SVG
 * @param {String} wrapperId 
 * @returns {HTMLDivElement} brandingWrapperNode
 */
function createOrbitBrandingSVGElement(wrapperId) {
  var brandingWrapperNode = document.createElement("div");
  brandingWrapperNode.id = wrapperId;
  brandingWrapperNode.innerHTML = '<svg class="fill-current" height="18" width="130"><use xlink:href="#orbit-brand" x="0" y="0" height="18" width="130"></use></svg>';
  return brandingWrapperNode;
}

/**
 * Uses
 *    - Creates and appends Orbit branding in appropriate sections based on the viewport dimensions (web vs mobile views)
 * @param {PaymentDetails} paymentDetails
 */
function renderBranding(paymentDetails) {
  if (paymentDetails.branding_visibility !== false) {
    ...
    var cartItemNode = document.getElementById("hyper-checkout-cart");
    if (cartItemNode instanceof HTMLDivElement) {
      var brandingWrapper = createOrbitBrandingSVGElement("powered-by-hyper");
      cartItemNode.appendChild(brandingWrapper);
    }

    // Append in document's body for mobile views
    var mobileBrandingWrapper = createOrbitBrandingSVGElement("hyper-footer");
