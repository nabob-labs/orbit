...
pub async fn get_decrypted_response_payload(
    ...
    let target_locker = locker_choice.unwrap_or(api_enums::LockerChoice::OrbitCardVault);
...
pub async fn mk_add_locker_request_hs(
    ...
    let mut url = match locker_choice {
        api_enums::LockerChoice::OrbitCardVault => locker.host.to_owned(),
    };
...
pub async fn mk_delete_card_request_hs(
    ...
    let jwe_payload =
        mk_basilisk_req(jwekey, &jws, api_enums::LockerChoice::OrbitCardVault).await?;
...
impl transformers::ForeignFrom<api_models::payment_methods::ConnectorTokenDetails>
    for orbit_domain_models::mandates::ConnectorTokenReferenceRecord
{
...
impl
    transformers::ForeignFrom<(
        ...
        orbit_domain_models::mandates::ConnectorTokenReferenceRecord,
    )> for api_models::payment_methods::ConnectorTokenDetails
{
    fn foreign_from(
        ...
        let orbit_domain_models::mandates::ConnectorTokenReferenceRecord {
            connector_token_request_reference_id,
            original_payment_authorized_amount,
            original_payment_authorized_currency,
            metadata,
