...
use orbit_constraint_graph as cgraph;
#[cfg(all(any(feature = "v1", feature = "v2"), not(feature = "customer_v2")))]
use orbit_domain_models::customer::CustomerUpdate;
            ...
            payment_methods::DataDuplicationCheck::MetaDataChanged => {
                if let Some(card) = req.card.clone() {
                    ...
                    let add_card_resp = add_card_hs(
                        ...
                        &customer_id,
                        merchant_account,
                        api::enums::LockerChoice::OrbitCardVault,
                        Some(
                            existing_pm
                                .locker_id
                                .as_ref()
...
> {
    ...
    let store_resp = add_card_to_hs_locker(
        ...
        &payload,
        customer_id,
        api_enums::LockerChoice::OrbitCardVault,
    )
    .await?;
...
> {
    ...
    logger::debug!("card added to orbit-card-vault");
...
pub async fn get_card_from_locker(
    ...
    let get_card_from_rs_locker_resp = common_utils::metrics::utils::record_operation_time(
        async {
            ...
            .await
            .change_context(errors::ApiErrorResponse::InternalServerError)
            .attach_printable("Failed while getting card from orbit card vault")
            .inspect_err(|_| {
                metrics::CARD_LOCKER_FAILURES.add(
                    1,
...
pub async fn delete_card_from_hs_locker<'a>(
    ...
    if !locker.mock_locker {
        call_locker_api::<payment_methods::DeleteCardResp>(
            ...
            request,
            "delete_card_from_locker",
            Some(api_enums::LockerChoice::OrbitCardVault),
        )
        .await
        .change_context(errors::VaultError::DeleteCardFailed)
    } else {
...
pub async fn do_list_customer_pm_fetch_customer_if_not_passed(
    ...
    if let Some(customer_id) = customer_id {
        ...
        let payment_intent: Option<orbit_domain_models::payments::PaymentIntent> =
            helpers::verify_payment_intent_time_and_client_secret(
                &state,
                &merchant_account,
                &key_store,
...
pub async fn list_customer_payment_method(
    ...
    for pm in resp.into_iter() {
        ...
        let orbit_token_data = pm_list_context
            .orbit_token_data
        ...
        .insert(intent_fulfillment_time, orbit_token_data, state)
        .await?;
    ...
    let payment_method_retrieval_context = match payment_method {
        enums::PaymentMethod::Card => {
            ...
            card_details.as_ref().map(|card| PaymentMethodListContext {
                ...
                orbit_token_data: is_payment_associated.then_some(
        ...
        enums::PaymentMethod::BankDebit => {
            ...
            bank_account_token_data.map(|data| {
                ...
                PaymentMethodListContext {
                    ...
                    #[cfg(feature = "payouts")]
                    bank_transfer_details: None,
                    orbit_token_data: is_payment_associated.then_some(token_data),
                }
            })
        }
        ...
        enums::PaymentMethod::Wallet => Some(PaymentMethodListContext {
            ...
            #[cfg(feature = "payouts")]
            bank_transfer_details: None,
            orbit_token_data: is_payment_associated
        ...
        enums::PaymentMethod::BankTransfer => Some(PaymentMethodListContext {
            ...
            orbit_token_data: parent_payment_method_token
