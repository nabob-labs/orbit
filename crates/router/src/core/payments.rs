...
use orbit_domain_models::payments::{payment_intent::CustomerData, ClickToPayMetaData};
#[cfg(feature = "v2")]
use orbit_domain_models::payments::{
    PaymentCaptureData, PaymentConfirmData, PaymentIntentData, PaymentStatusData,
};
#[cfg(feature = "v2")]
use orbit_domain_models::router_response_types::RedirectForm;
pub use orbit_domain_models::{
...
where
    ...
    RouterData<F, FData, router_types::PaymentsResponseData>:
        orbit_domain_models::router_data::TrackerPostUpdateObjects<F, FData, D>,
    ...
    FData: Send + Sync + Clone,
{
    ...
    if let Some(connector_details) = connector {
        ...
        payment_data
            ...
            .async_map(|key_for_orbit_token| async move {
                if key_for_orbit_token
                    .should_delete_payment_method_token(payment_intent_status)
                {
                    let _ = key_for_orbit_token.delete(state).await;
                }
            })
            .await;
    } else {
...
pub async fn call_surcharge_decision_management_for_session_flow(
    ...
    _billing_address: Option<orbit_domain_models::address::Address>,
...
pub async fn call_surcharge_decision_management_for_session_flow(
    ...
    billing_address: Option<orbit_domain_models::address::Address>,
        ...
        services::api::ConnectorIntegration<F, RouterDReq, router_types::PaymentsResponseData>,
{
    ...
    *payment_data = pd;
    ...
    // there should be no error in request construction from orbit end
    (_, *payment_data) = operation
        .to_update_tracker()?
        .update_trackers(
            state,
    ...
    match &tokenization_action {
        ...
        | TokenizationAction::TokenizeInConnectorAndApplepayPreDecrypt(
            ...
            let apple_pay_predecrypt = apple_pay_data
                .parse_value::<orbit_domain_models::router_data::ApplePayPredecryptData>(
        ...
        TokenizationAction::DecryptPazeToken(payment_processing_details) => {
            ...
            let paze_decrypted_data = paze_data
                .parse_value::<orbit_domain_models::router_data::PazeDecryptedData>(
                    "PazeDecryptedData",
                )
                .change_context(errors::ApiErrorResponse::InternalServerError)
    ...
    let final_recipient_data = if let Some(id) = recipient_id {
        if contains {
            ...
            let locker_resp = cards::get_payment_method_from_hs_locker(
                ...
                merchant_account.get_id(),
                id.as_str(),
                Some(enums::LockerChoice::OrbitCardVault),
            )
            .await
            .change_context(errors::ApiErrorResponse::InternalServerError)
...
pub async fn get_session_token_for_click_to_pay(
    ...
    payment_intent: &orbit_domain_models::payments::PaymentIntent,
                    ...
                    api_models::payments::ApplePayCombinedMetadata::Manual {
                            ...
                            session_token_data.payment_processing_details_at
                        {
                            match manual_payment_processing_details_at {
                                payments_api::PaymentProcessingDetailsAt::Orbit(
                                    payment_processing_details,
    ...
    F: Clone,
{
    ...
    pub recurring_mandate_payment_data:
        Option<orbit_domain_models::router_data::RecurringMandatePaymentData>,
    ...
    pub card_testing_guard_data:
        Option<orbit_domain_models::card_testing_guard_data::CardTestingGuardData>,
    pub vault_operation: Option<domain_payments::VaultOperation>,
}

#[derive(Clone, serde::Serialize, Debug)]
pub struct TaxData {
    pub shipping_details: orbit_domain_models::address::Address,
...
pub async fn list_payments(
    ...
    use orbit_domain_models::errors::StorageError;
...
async fn get_eligible_connector_for_nti<T: core_routing::GetRoutableConnectorsForChoice, F, D>(
    ...
    orbit_domain_models::payment_method_data::CardDetailsForNetworkTransactionId,
    api::ConnectorData,
)>
where
    F: Send + Clone,
    ...
    D: OperationSessionGetters<F> + OperationSessionSetters<F> + Send + Sync + Clone,
{
    ...
    payment_data.set_payment_method_data(Some(
        orbit_domain_models::payment_method_data::PaymentMethodData::CardDetailsForNetworkTransactionId(card_details_for_network_transaction_id),
    ));
                ...
                if let Some(mandate_reference_record) = connector_mandate_details.clone()
                            ...
                            .get(merchant_connector_id)
                        {
                            ...
                            payment_data.set_recurring_mandate_payment_data(
                                orbit_domain_models::router_data::RecurringMandatePaymentData {
...
pub trait OperationSessionGetters<F> {
    ...
    fn get_billing_address(&self) -> Option<orbit_domain_models::address::Address>;
