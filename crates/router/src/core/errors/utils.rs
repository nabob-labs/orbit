...
impl<T> StorageErrorExt<T, errors::ApiErrorResponse>
    for error_stack::Result<T, orbit_domain_models::errors::StorageError>
{
    #[track_caller]
    fn to_not_found_response(
        self,
        ...
        self.map_err(|err| {
            let new_err = match err.current_context() {
                orbit_domain_models::errors::StorageError::ValueNotFound(_) => {
                    not_found_response
                }
                orbit_domain_models::errors::StorageError::CustomerRedacted => {
    ...
    fn to_duplicate_response(
        ...
        self.map_err(|err| {
            let new_err = match err.current_context() {
                orbit_domain_models::errors::StorageError::DuplicateValue { .. } => {
                    duplicate_response
                }
                _ => errors::ApiErrorResponse::InternalServerError,
            };
    ...
    fn to_payment_failed_response(self) -> error_stack::Result<T, errors::ApiErrorResponse> {
        self.map_err(|err| {
            let error = match err.current_context() {
                ...
                errors::ConnectorError::CurrencyNotSupported { message, connector} => errors::ApiErrorResponse::CurrencyNotSupported { message: format!("Credentials for the currency {message} are not configured with the connector {connector}/orbit") },
