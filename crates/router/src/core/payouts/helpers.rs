...
use orbit_domain_models::type_encryption::{crypto_operation, CryptoOperation};
...
pub async fn make_payout_method_data(
    ...
    let db = &*state.store;
    let orbit_token = if let Some(payout_token) = payout_token {
        if payout_token.starts_with("temporary_token_") {
            ...
            let payment_token_data = orbit_token
                .clone()
