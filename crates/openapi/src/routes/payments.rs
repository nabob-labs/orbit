...
/// 4. Payments - Capture.
///
/// You will require the 'API - Key' from the Orbit dashboard to make the first call, and use the 'client secret' returned in this API along with your 'publishable key' to make subsequent API calls from your client.
///
...
///
/// Creates a session object or a session token for wallets like Apple Pay, Google Pay, etc. These tokens are used by Orbit's SDK to initiate these wallets' SDK.
#[utoipa::path(
  post,
...
///
/// You will require the 'API - Key' from the Orbit dashboard to make the call.
#[utoipa::path(
  get,
